...
**click-to-cancel for your buffs** plus several aura bug fixes. Everything below sits on top of
stock SUF; nothing else about the addon changes.

## Click-to-cancel personal buffs

Click one of your own buffs on the **player** frame to cancel it: food, flasks, Well Fed, weapon
oils, anything the game lets you cancel. It works like the default Blizzard buff bar, but on your
SUF auras, with all your filtering and layout kept intact.

**How to use**
1. Enable the player **Buffs** frame and make sure it is not set to click-through
   (Units, then Player, then Auras, then Buffs).
2. Out of combat, click a buff icon to remove it.

**Good to know**
- **Out of combat only.** WoW blocks buff cancellation during combat. That is a hard Blizzard
...
- **Anchor position sticks again.** Choosing (say) *Bottom Left* for an aura frame now docks
  where it should. A stale "forced anchor" left over from the buffs/debuffs *anchor-on* pairing
  used to override your choice until the next reload.
...
- **Global panel actually applies.** In the **Global** section, aura-frame settings (enable,
  position, size, filter, and so on) and **Test mode** now fan out to every selected unit.
  Previously they silently did nothing.
